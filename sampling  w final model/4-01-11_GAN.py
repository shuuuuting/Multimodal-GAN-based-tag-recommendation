...
class GAN(object):
    def __init__(self, num_tags):
        ...
        self.g_optimizer = Adam(lr=0.00002, beta_1=0.5, decay=8e-8)
        ...
        self.stacked_G_D.compile(loss=['BinaryCrossentropy', 'BinaryCrossentropy'], loss_weights=[10,0.5], optimizer=self.g_optimizer) #, loss_weights=[50,0.5]
        ...
        for e in range(epochs):
            ...
            for g_step in range(g_steps):
                ...
                for cnt in range(iters):
                    ...
                    x_gen_batch = [np.append(real_x[0][lwr:upr], tobegen_x[0],axis=0), np.append(real_x[1][lwr:upr], tobegen_x[1],axis=0)
                                    , np.append(real_x[2][lwr:upr], tobegen_x[2],axis=0), np.append(real_x[3][lwr:upr], tobegen_x[3],axis=0)]
                    y_mislabled = np.ones((batch_size, 1))
                    g_loss = self.stacked_G_D.train_on_batch(x_gen_batch, [np.append(real_tags[lwr:upr], tobegen_y, axis=0), y_mislabled])
                    lwr += batch_size//2
                    upr += batch_size//2

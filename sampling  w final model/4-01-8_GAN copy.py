...
class GAN(object):
    ...
    def __generator(self):
        ...
        inputs_group_cnn = Input(shape=(seq_length, embedding_size))
        ...
        eFeature.set_shape((inputs_group_cnn.shape[0], seq_length, embedding_size))
    ...
    def train(self, image_train, text_train, group_pop_train, group_cnn_train, tag_train, epochs, batch_size):
        ...
        for e in range(epochs):
            ...
            acc_tr, _, _, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train, embedding_train, lda_train, lda_tr_arr]), TopK) 
            acc_te, _, _, _, _, _ = evaluation(tag_test, self.G.predict(x=[image_test, embedding_test, lda_test, lda_te_arr]), TopK) 
...
#real_tags = (real_tags*0.2) + np.random.random(real_tags.shape)*0.001
#real_tags = (real_tags*0.6) + np.random.random(real_tags.shape)*0.35
f_real_tags = (real_tags*0.01*np.random.randint(41,size=real_tags.shape)) + np.random.random(real_tags.shape)*0.001

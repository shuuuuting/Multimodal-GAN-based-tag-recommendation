...
class DataGen(Sequence):
    def __init__(self, image_train, text_train, tag_train, tot_ex, batch_size):
	    ...
	    self.globalidx = np.random.choice(self.idlist, int(self.tot_ex*0.7)) #era global training set
	    self.G_sampleprob = np.ones(len(self.globalidx)) / len(self.globalidx)
	    self.D_sampleprob = np.ones(len(self.globalidx)) / len(self.globalidx)
	    self.knownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25)) #random再抽50%作為epoch known set
	    self.G_unknownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25), p = self.G_sampleprob) #以抽樣機率再抽50%作為epoch G's unknown set
	    self.D_unknownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25), p = self.D_sampleprob) #以抽樣機率再抽50%作為epoch D's unknown set
    ...
    def on_epoch_end(self): #更新sample prob./抽known&unknown set
	    ...
	    if np.isnan(G_loss).any():
	        ...
	        self.G_sampleprob = (1/G_loss[self.globalidx]) / sum(1/G_loss[self.globalidx])
	    if np.isnan(G_loss).any():
	        self.D_sampleprob = np.ones(len(self.globalidx)) / len(self.globalidx)
	    else:
	        self.D_sampleprob = (1/G_classloss[self.globalidx]) / sum(1/G_classloss[self.globalidx])
	    self.knownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25)) #random再抽50%作為epoch known set
	    self.G_unknownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25), p = self.G_sampleprob) #以抽樣再抽50%作為epoch unknown set
	    self.D_unknownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25), p = self.D_sampleprob) #以抽樣再抽50%作為epoch unknown set
	    self.lwr = 0
...
#%%GAN sampling strategy 7
#era(5 epochs): global 0.7 
#epoch: knownset random from global / unknownset chose by sample prob from global 
...
#precision:  0.14144620811287476
#recall:  0.6135158590185045
#f1:  0.21737978299807711

#1/loss
#acc:  0.7178130511463845
#precision:  0.1419753086419753
#recall:  0.6004724111866969
#f1:  0.21687421640654395

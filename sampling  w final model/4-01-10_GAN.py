...
class GAN(object):
    def __init__(self, num_tags):
        ...
        self.stacked_G_D.compile(loss=[myLossFunc, 'BinaryCrossentropy'], optimizer=self.g_optimizer) #, loss_weights=[100,0.5]
    ...
    def __generator(self):
        ...
        model = Model(inputs=[inputs_img, inputs_text, inputs_group_pop, inputs_group_cnn], outputs=[Softmax])
        #model.summary()
        return model 
    ...
    def train(self, image_train, text_train, group_pop_train, group_cnn_train, tag_train, epochs, batch_size):
        ...
        for e in range(epochs):
            for cnt in range(iters):
                ...
                #real_tags = (real_tags*0.6) + np.random.random(real_tags.shape)*0.4
                real_tags = (real_tags*0.2) + np.random.random(real_tags.shape)*0.001
                #real_tags = (real_tags*0.01*np.random.randint(71,size=real_tags.shape)) + np.random.random(real_tags.shape)*0.001
...
real_tags = (real_tags*0.2) + np.random.random(real_tags.shape)*0.001
#real_tags = (real_tags*0.01*np.random.randint(41,size=real_tags.shape)) + np.random.random(real_tags.shape)*0.001

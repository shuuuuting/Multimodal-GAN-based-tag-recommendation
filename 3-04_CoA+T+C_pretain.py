...
class coAttention_alt(Layer):
    ...
    def call(self, x, mask=None): #call:編寫layer邏輯的function，執行forward propagation
        ...
        # phase 2-2: co-guided channel feature 
...
def modelDef():
    ...
    inputs_channel = Input(shape=(seq_length,embedding_size))
    inputs_title = Input(shape=(title_length,embedding_size)) 
    
    iCNN = load_model('pretrained_iCNN.h5')
    tCNN = load_model('pretrained_tCNN.h5')
    cCNN = load_model('pretrained_cCNN.h5')
    ...
    cCNN = Model(inputs=cCNN.input, outputs=cCNN.get_layer('ccnn').output)
    ...
    cFeature = cCNN(inputs_channel)
    cFeature = SeqSelfAttention(attention_activation='sigmoid')(cFeature)
    cFeature.set_shape((inputs_channel.shape[0],seq_length,embedding_size))
    
    #eFeature = eCNN(inputs_title)
    ...
    co_feature = coAttention_alt(dim_k=dim_k)([iFeature, tFeature, cFeature, eFeature])
...
model = modelDef()
history = model.fit(x=[image_train, embedding_train, embedding_train, embedding_title_train], #group_train
                    y=tag_train.astype(np.float32),
                    batch_size=batch_size,
                    epochs=50,
                    verbose=1,)
                    #validation_data=([image_test, embedding_test, embedding_test, embedding_title_test], tag_test.astype(np.float32)), 
                    #callbacks=[early_stopping])
y_pred = model.predict(x=[image_test, embedding_test, embedding_test, embedding_title_test]) #group_test
...
# + phase 2-2 i+t guide title embedding(pretrained word2vec) 
#acc:  0.7971781305114638
#precision:  0.15767195767195769
#recall:  0.6920120517342739
#f1:  0.24277661224358033

#alpha=0.5/beta=0.5
#acc:  0.7918871252204586
#precision:  0.15767195767195769
#recall:  0.6964996080736822
#f1:  0.24306868070607485

#title用word2vec model產生embedding不會比較好!!
...
class DataGen(Sequence):
    ...
    def on_epoch_end(self): #重新抽globalset＆更新sample prob.
	    ...
	    G_out, GAN_out = gan.stacked_G_D.predict([image_train, text_train])
    ...
    def __data_generation(self, indices):
        indices.sort()
        X = [image_train[indices], text_train[indices]]
        y = tag_train[indices]
        return (X,y)

datagen = DataGen(image_train, text_train, tag_train, tot_ex=len(image_train), batch_size=64)
...
class GAN(object):
    ...
    def train(self, image_train, text_train, tag_train, epochs, batch_size):
        ...
        for e in range(epochs):
            ...
            acc_K, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train, text_train]), TopK) 
...
batch_size = 64
gan = GAN(num_tags)
d_history, g_history = gan.train(image_train, text_train, tag_train.astype(np.float32), epochs=epochs, batch_size=batch_size)

#%%
y_pred = gan.G.predict(x=[image_test, text_test])
...
#%%GAN sampling strategy 11
#跟8差在他會在一個epoch把globalset的樣本全部跑完
...
#precision:  0.15440917107583774
#recall:  0.669724391814339
#f1:  0.23700113466113437
#st8的globalset改成不是random抽而是一個batch跑完整個globalset

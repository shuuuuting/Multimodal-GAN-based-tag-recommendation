...
class DataGen(Sequence):
    def __init__(self, image_train, text_train, tag_train, tot_ex, batch_size):
	    ...
	    self.G_sampleprob = np.ones(int(self.tot_ex)) / int(self.tot_ex)
	    self.D_sampleprob = np.ones(int(self.tot_ex)) / int(self.tot_ex)
	    self.knownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25)) #random再抽50%作為epoch known set
	    self.G_unknownidx = np.random.choice(self.idlist, int(self.tot_ex*0.25), p = self.G_sampleprob) #以抽樣機率再抽50%作為epoch G's unknown set
	    self.D_unknownidx = np.random.choice(self.idlist, int(self.tot_ex*0.25), p = self.D_sampleprob) #以抽樣機率再抽50%作為epoch D's unknown set
    ...
    def on_epoch_end(self): #更新sample prob./抽known&unknown set
	    ...
	    self.knownidx = np.random.choice(self.globalidx, int(self.tot_ex*0.25)) #random再抽50%作為epoch known set
	    self.G_unknownidx = np.random.choice(self.idlist, int(self.tot_ex*0.25), p = self.G_sampleprob) #以抽樣再抽50%作為epoch unknown set
	    self.D_unknownidx = np.random.choice(self.idlist, int(self.tot_ex*0.25), p = self.D_sampleprob) #以抽樣再抽50%作為epoch unknown set
	    self.lwr = 0
...
#era(5 epochs): global 0.5
#epoch: knownset random from global / unknownset chose by sample prob from entire trainingset
...
#myLossFunc
#loss
#acc:  0.7583774250440917
#precision:  0.14673721340388007
#recall:  0.643778519638308
#f1:  0.22569278675147925

#1/loss
#acc:  0.7557319223985891
#precision:  0.14673721340388007
#recall:  0.6391772346798802
#f1:  0.22563825747192662

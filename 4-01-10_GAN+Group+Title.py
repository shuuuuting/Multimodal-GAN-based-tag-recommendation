...
datagen = DataGen(image_train, embedding_train, lda_train, lda_tr_arr, tag_train, tot_ex=len(image_train), batch_size=64)
...
class GAN(object):
    def __init__(self, num_tags):
        ...
        #self.G.load_weights('pretrained_G_w.h5')
    ...
    def __generator(self):
        ...
        inputs_group = Input(shape=(seq_length, embedding_size)) #embedding
        ...
        gFeature = groupFeature(inputs_group)
        ...
        Sigmoid = Dense(num_tags, activation="sigmoid", use_bias=True)(dropout) #tag_prob
        model = Model(inputs=[inputs_img, inputs_text, inputs_group, inputs_title], outputs=[Softmax])
        #model.summary()
        return model 
    ...
    def train(self, image_train, text_train, group_train, title_train, tag_train, epochs, batch_size):
        ...
        for e in range(epochs):
            for cnt in range(iters):
                ...
                x_gen_batch = [np.append(real_x[0], tobegen_x[0],axis=0), np.append(real_x[1], tobegen_x[1],axis=0), np.append(real_x[2], tobegen_x[2],axis=0), np.append(real_x[3], tobegen_x[3],axis=0)]
            ...
            acc_tr, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train, embedding_train, lda_train, lda_tr_arr]), TopK) 
            acc_te, _, _, _ = evaluation(tag_test, self.G.predict(x=[image_test, embedding_test, lda_test, lda_te_arr]), TopK)            
...
batch_size = 64
gan = GAN(num_tags)
d_history, g_history = gan.train(image_train, embedding_train, lda_train, lda_tr_arr, tag_train.astype(np.float32), epochs=epochs, batch_size=batch_size)

#%%
y_pred = gan.G.predict(x=[image_test, embedding_test, lda_test, lda_te_arr])
...
#precision:  0.1551146384479718
#recall:  0.6735141793342851
#f1:  0.23840363528554595

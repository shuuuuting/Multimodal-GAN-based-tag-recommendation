...
class DataGen(Sequence):
    def __init__(self, image_train, tag_train, tot_ex, batch_size):
    ...
    def __data_generation(self, indices):
        indices.sort()
        X = image_train[indices]
        y = tag_train[indices]
        return (X,y)

datagen = DataGen(image_train, tag_train, tot_ex=len(image_train), batch_size=128)
...
class GAN(object):
    ...
    def __generator(self):
        ...
        iFeature = Reshape(target_shape=(num_region, 512))(inputs_img)
        iFeature = Dense(embedding_size, activation="tanh", use_bias=False)(iFeature) #single layer to convert each img vector into a new same dim vector as text feature vector
        iFeature = AveragePooling1D(pool_size=num_region, padding="same")(iFeature)
        iFeature = Flatten()(iFeature)
        iFeature = Dense(600, activation='relu')(iFeature)
        tag_prob = Dense(num_tags, activation='sigmoid')(iFeature)
        model = Model(inputs=[inputs_img], outputs=[tag_prob])
        #model.summary()
        return model 
    ...
    def train(self, image_train, tag_train, epochs, batch_size):
        dlist, glist = [], []
        global datagen
        ...
        for e in range(epochs):
            for cnt in range(iters):
                ...
                x_gen_batch = [np.append(real_x, image_train[randidx],axis=0)]
            ...
            acc_K, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train]), TopK) 
            print('epoch: %d, [d_loss: %f], [g_loss: %f], [acc: %f]' % (e+1, d_loss, g_loss[0], acc_K)) 
            print('[class_loss: %f], [adv_loss: %f]' % (g_loss[1], g_loss[2])) 
            dlist.append(g_loss[1])#d_loss) 
            glist.append(g_loss[2])#g_loss[0])
...
batch_size = 128
gan = GAN(num_tags)
d_history, g_history = gan.train(image_train, tag_train.astype(np.float32), epochs=epochs, batch_size=batch_size)

#%%
y_pred = gan.G.predict(x=[image_test])
...
#precision:  0.04241622574955908
#recall:  0.148969443744576
#f1:  0.06277250212537777

...
class DataGen(Sequence):
    ...
    def on_era_end(self): #重新抽globalset
	    G_out, GAN_out = gan.stacked_G_D.predict([self.image_train, self.text_train, self.title_train])
    ...
    def __data_generation(self, indices):
        indices.sort()
        X = [self.image_train[indices], self.text_train[indices], self.title_train[indices]]
        y = self.tag_train[indices]
        return (X,y)

datagen = DataGen(image_train, embedding_train, embedding_title_train, tag_train, tot_ex=len(image_train), batch_size=64)
...
class GAN(object):
    ...
    def __generator(self):
        ...
        inputs_group = Input(shape=(title_length, embedding_size)) #embedding
        ...
        gFeature.set_shape((inputs_group.shape[0], title_length, embedding_size))
        ...
        model = Model(inputs=[inputs_img, inputs_text, inputs_group], outputs=[Softmax])
        #model.summary()
        return model 
    ...
    def train(self, image_train, text_train, title_train, tag_train, epochs, batch_size):
        ...
        for e in range(epochs):
            for cnt in range(iters):
                ...
                x_gen_batch = [np.append(real_x[0], tobegen_x[0],axis=0), np.append(real_x[1], tobegen_x[1],axis=0), np.append(real_x[2], tobegen_x[2],axis=0)]
            ...
            acc_tr, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train, embedding_train, embedding_title_train]), TopK) 
            acc_te, _, _, _ = evaluation(tag_test, self.G.predict(x=[image_test, embedding_test, embedding_title_test]), TopK)            
...
batch_size = 64
gan = GAN(num_tags)
d_history, g_history = gan.train(image_train, embedding_train, embedding_title_train, tag_train.astype(np.float32), epochs=epochs, batch_size=batch_size)

#%%
y_pred = gan.G.predict(x=[image_test, embedding_test, embedding_title_test])

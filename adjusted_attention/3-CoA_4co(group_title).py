...
for i in range(round):
    print('round', i+1)
    model = modelDef()
    history = model.fit(x=[image_train, text_title_train, lda_train, lda_title_tr_arr], 
    ...
    y_pred = model.predict(x=[image_test, text_title_test, lda_test, lda_title_te_arr]) 

...
class DataGen(Sequence):
    def __init__(self, image_train, text_train, group_train, tag_train, tot_ex, batch_size):
...
datagen = DataGen(image_train, text_train, title_train, tag_train, tot_ex=len(image_train), batch_size=batch_size)
...
class GAN(object):
    def __init__(self, num_tags):
        ...
        self.stacked_G_D.compile(loss=[myLossFunc, myLossFunc], loss_weights = [0.2,0.8], optimizer=self.g_optimizer)
        ...
        for e in range(epochs):
            for cnt in range(iters):
                ...
                x_gen_batch = [np.append(real_x[0], image_train[randidx],axis=0), np.append(real_x[1], text_train[randidx],axis=0)
                                , np.append(real_x[2], title_train[randidx],axis=0)]
...
#precision:  0.16331569664903
#recall:  0.7147437781697041
#f1:  0.25128386701703165

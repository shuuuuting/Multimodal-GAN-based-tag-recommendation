...
class coAttention_alt(Layer):
    ...
    def call(self, x, mask=None): #call:編寫layer邏輯的function，執行forward propagation
        ...
        # phase 2-1: group doc2vec guided text feature 
...
def groupFeature(X):
    X = Embedding(input_dim=num_words, output_dim=embedding_size,
                           mask_zero=True, input_length=10)(X) #seq_length:一次輸入带有的詞彙個數
...
def modelDef():
    ...
    inputs_title = Input(shape=(seq_length+title_length,embedding_size)) #embedding #group的話是50，title的話是10
    ...
    eFeature = titleFeature(inputs_title)
    ...
    model = Model(inputs=[inputs_img, inputs_text, inputs_group, inputs_title],
                  outputs=[Softmax])
...
model = modelDef()
groupd2v_title_tr = group_train + np.mean(embedding_title_train,axis=1)
groupd2v_title_te = group_test + np.mean(embedding_title_test,axis=1)
history = model.fit(x=[image_train, embedding_train, group_train, group_title_tr_arr], #embedding_train
                    y=tag_train.astype(np.float32),
                    batch_size=batch_size,
                    epochs=50,
                    verbose=1,)
                    #validation_data=([image_test, embedding_test, group_test, embedding_title_train], tag_test.astype(np.float32)), 
                    #callbacks=[early_stopping])
y_pred = model.predict(x=[image_test, embedding_test, group_test, group_title_te_arr]) #embedding_test
...
#%% phase 2-1 group doc2vec guide text 
# + phase 2-2 i+t guide title embedding 
#acc:  0.7927689594356261
#precision:  0.1575837742504409
#recall:  0.6902417345539039
#f1:  0.24256189273511997

#alpha=0.5/beta=0.5
#acc:  0.8077601410934744
#precision:  0.1615520282186949
#recall:  0.7103696005151031
#f1:  0.2488340425919259

#alpha=0.5/beta=0.8
#acc:  0.7962962962962963
#precision:  0.158994708994709
#recall:  0.6976242966322331
#f1:  0.24459070905845523

#alpha=0.8/beta=0.8
#acc:  0.7954144620811288
#precision:  0.15943562610229278
#recall:  0.699739648945998
#f1:  0.24539605868110956

#alpha=0.8/beta=0.5
#acc:  0.7962962962962963
#precision:  0.15837742504409172
#recall:  0.6953588225413622
#f1:  0.24377536579835626

#doc2vec pretrain by word2vec
#alpha=0.5/beta=0.5
#acc:  0.7927689594356261
#precision:  0.15961199294532627
#recall:  0.6997392990117858
#f1:  0.24562072857484588

#embedding_train, doc2vec pretrain, embedding_title_train / no selfattn
#acc:  0.8350970017636684
#precision:  0.16552028218694884
#recall:  0.737119621511156
#f1:  0.25563178186239105

#embedding_train, doc2vec pretrain, embedding_title_train / selfattn
#acc:  0.8783068783068783
#precision:  0.17354497354497356
#recall:  0.7825876235267769
#f1:  0.26858321092886267
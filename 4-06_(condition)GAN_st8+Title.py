...
class DataGen(Sequence):
    def __init__(self, image_train, embedding_train, title_train, tag_train, tot_ex, batch_size):
    ...
    def on_epoch_end(self): #重新抽globalset＆更新sample prob.
	    ...
	    G_out, GAN_out = gan.stacked_G_D.predict([image_train, embedding_train, title_train])
    ...
    def __data_generation(self, indices):
        indices.sort()
        X = [image_train[indices], embedding_train[indices], title_train[indices]]
        y = tag_train[indices]
        return (X,y)

datagen = DataGen(image_train, embedding_train, title_train, tag_train, tot_ex=len(image_train), batch_size=64)
...
class GAN(object):
    ...
    def __generator(self):
        ...
        inputs_text = Input(shape=(seq_length,embedding))
    ...
    def train(self, image_train, embedding_train, title_train, tag_train, epochs, batch_size):
        dlist, glist = [], []
        global datagen
        ...
        for e in range(epochs):
            ...
            acc_K, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train, embedding_train, title_train]), TopK) 
...
batch_size = 64
gan = GAN(num_tags)
d_history, g_history = gan.train(image_train, embedding_train, title_train, tag_train.astype(np.float32), epochs=epochs, batch_size=batch_size)

#%%
y_pred = gan.G.predict(x=[image_test, embedding_test, title_test])
...
#precision:  0.16208112874779543
#recall:  0.7220336076817557
#f1:  0.25023125497391224

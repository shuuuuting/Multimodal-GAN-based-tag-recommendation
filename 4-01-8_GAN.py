...
class GAN(object):
    def __init__(self, num_tags):
        ...
        self.stacked_G_D.compile(loss=['BinaryCrossentropy', 'BinaryCrossentropy'], loss_weights=[100,0.5], optimizer=self.g_optimizer)
    ...
    def train(self, image_train, text_train, tag_train, epochs, batch_size):
        ...
        for e in range(epochs):
            ...
            acc_tr, _, _, _, _, _ = evaluation(tag_train, self.G.predict(x=[image_train, embedding_train]), TopK) 
            acc_te, _, _, _, _, _ = evaluation(tag_test, self.G.predict(x=[image_test, embedding_test]), TopK) 
